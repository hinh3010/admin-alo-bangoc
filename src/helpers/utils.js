...
export const formatTime = time => {
  const timeZoneDecode = new Date(time);
  const time_zone = new Date().getTimezoneOffset();

  const d1 = new Date().getTime();
  const d2 = new Date(timeZoneDecode.getTime() - time_zone).getTime();

  const getTimeForPost = Math.floor(Math.abs(d1 - d2) / 1000);

  let textShowView;
  if (getTimeForPost < 3600) {
    const minuteGet = getTimeForPost / 60;
    if (minuteGet < 1) textShowView = 'Vừa xong';
    else textShowView = `${Math.floor(minuteGet)} phút trước`;
  } else if (getTimeForPost >= 3600 && getTimeForPost < 86400) {
    const hourGet = getTimeForPost / 3600;
    textShowView = `${Math.floor(hourGet)} giờ trước`;
  } else if (getTimeForPost > 86400 && getTimeForPost < 172800) {
    const options1 = { hour: '2-digit', minute: '2-digit' };
    const formatter1 = new Intl.DateTimeFormat('vi-VI', options1);
    // let timeFirstDayGet = moment(timeZoneDecode).format('HH:MM');
    const timeFirstDayGet = formatter1.format(d2);
    textShowView = `Hôm qua, ${timeFirstDayGet}`;
  } else {
    // textShowView = moment(timeZoneDecode).format("DD-MM-YYYY");
    const options2 = { year: 'numeric', month: '2-digit', day: '2-digit' };
    const formatter2 = new Intl.DateTimeFormat('vi-VI', options2);
    textShowView = formatter2.format(d2).replace(/\//gm, '-');
  }
