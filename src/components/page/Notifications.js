...
const Notifications = ({ items = rawNotifications.length, children }) => {
  ...
  const { listNoti } = useContext(AppContext);
  ...
  return (
    <Card className="h-100">
      <FalconCardHeader title="Danh sách thông báo">
        <div className="fs--1">
      ...
      <CardBody className="p-0">
        {loading ? (
          ...
          listNoti.slice(0, items).map((notification, index) => <BoxNoti {...notification} key={index} />)
        ) : (
